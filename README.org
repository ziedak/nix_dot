#+title: zied' NixOS & Nix-Darwin System Configuration Flake
...
#+begin_src
  ...
  # git clone https://github.com/ziedAkremi/nixos-config /mnt/etc/nixos/<name>
...
6. Rebuilds:
   - ~$ sudo nixos-rebuild switch --flake <config path>#<host>~
   - For example ~$ sudo nixos-rebuild switch --flake ~/.setup#zied~
...
#+begin_src
  ...
  $ git clone https://github.com/ziedAkremi/nixos-config ~/.setup
  $ cd ~/.setup
